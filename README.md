...
* 사용자가 검색한 결과를 input값으로 받아서 json data와 비교할 때 match 메소드를 사용했는데 이때 서버로 정보를 보내는 역할을 하는 Form 원리를 이해할 수 있었습니다. 또한 input창에 입력을 마치고 enter를 치면 input창에 커서가 focus되고 빈칸으로 바뀌도록 할 때 시행착오를 거치면서 onChange() 함수와 input의 속성을 이용하는 법을 익힐 수 있었습니다.
