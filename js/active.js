...
import getFindMe from './getFindMe';
...
window.addEventListener('DOMContentLoaded', function () {
    ...
    function todayList() {
        ...
        dragList(-350, item, listBox, ulToday);//오늘의 밀롱가 드레그하기
    }//todaylist
    ...
    function dataFun(e) {
        ...
        input.addEventListener('change', function (e) {
            ...
            posChoice = [];
            getFindMe(mapContainer, mapOption, map);
            listLen = 0, idxList = 0;
    ...
    function dragList(moveX, listLen, item, listBox, ulEle) {
        listLen = item.length;
        let isDown = false;
        let startX;
        let endX;
    ...
    function mapSearch() {
        const listBox = document.querySelector('.listbox'),
            item = listBox.querySelectorAll('.item');
        dragList(-350, listLen, idxList, item, listBox, ulEle);//map안의 리스트 드레그하기
        ...
        geocoder.addressSearch(addressChoice[0].textContent, function (result, status) {
            ...
            if (status === kakao.maps.services.Status.OK) {
                markerEvent(posChoice, mapContainer, mapOption, map, idxList);
                findMeBtn.addEventListener('click', getFindMe);
                regionNow(map, geocoder);
            }
        });
    };
